...
"""Launcher for the btm-read-pdf workspace member.

uv resolves workspace and path sources lexically, and an agent reaches this
skill through an alias directory such as `.claude/skills/read-pdf/`, where a
...
needed. The program itself lives in `../src/btm_read_pdf/`.
"""
...
os.execvpe(
    "uv",
    ["uv", "run", "--project", str(member), "btm-read-pdf", *sys.argv[1:]],
    environment,
)
