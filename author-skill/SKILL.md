...
<system_directives>
  ...
  <script_design>
    ...
    <rule>Logic shared across bundled scripts lives once, in the repository kernel package `btm-corekit` under `.corekit/`. A consumer script declares `dependencies = ["btm-corekit"]` with the editable uv source `{ path = "../.corekit", editable = true }`, and its skill carries a dotted `.corekit` symlink to the repository kernel (the gate derives it). Consumers never redefine kernel symbols; such a skill's `compatibility` notes that it runs from a full repository checkout.</rule>
