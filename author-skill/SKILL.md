...
<system_directives>
  ...
  <script_design>
    ...
    <rule>Keep the command surface uniform: one record is a batch of one, sibling record kinds share one plural-array container schema, and every subcommand addresses its subject the same way. Each asymmetry costs the consuming agent a re-read of help text.</rule>
