...
"""Launcher for the btm-lit-review workspace member.

uv resolves workspace and path sources lexically, and an agent reaches this
skill through an alias directory such as `.claude/skills/lit-review/`, where a
...
needed. The program itself lives in `../src/btm_lit_review/`.
"""
...
os.execvpe(
    "uv",
    ["uv", "run", "--project", str(member), "btm-lit-review", *sys.argv[1:]],
    environment,
)
