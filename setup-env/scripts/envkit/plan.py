...
from .model import (DenvError, EnvDelta, Host, Layout, OS, Spec,
                    merge_deltas)
...
def _base_env(layout: Layout, host: Host, spec: Spec) -> EnvDelta:
    ...
    if host.os is OS.WINDOWS:
        vars_ += [("TEMP", str(layout.tmp)), ("TMP", str(layout.tmp)),
                  ("USERPROFILE", str(layout.home))]
    else:
        vars_ += [("TMPDIR", str(layout.tmp)),
    ...
    return EnvDelta(
        vars=tuple(vars_),
        path=(layout.shims,),
        unset=frozenset({"CONDA_DEFAULT_ENV", "CONDA_PREFIX", "CONDA_SHLVL",
                         "VIRTUAL_ENV"}),
    )
...
def make_plan(spec: Spec, host: Host, layout: Layout) -> Plan:
    ...
    for step in gathered:
        ...
        if prior is not None and prior.platform != platform:
            raise DenvError(f"prefix {step.prefix_rel} claimed for two "
