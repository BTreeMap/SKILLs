...
from .catalog import GOOGLE_MAVEN, qemu_steps
from .model import (CondaPlatform, DenvError, Host, Layout, OS, QemuUser,
...
from .steps import (Aapt2Shim, AndroidSdk, BindGradleProject, CompilerShims,
                    CondaEnv, Fetch, GhcupToolchain, UvShim, UvVenv)

MICROMAMBA_VERSION = "2.9.0-0"
MICROMAMBA_RELEASES = ("https://github.com/mamba-org/micromamba-releases/"
                       "releases/download")
...
def fetch(url: str, target: Path) -> None:
    ...
    with urllib.request.urlopen(request, context=_SSL) as src, \
...
def verify_sha256(path: Path, expected: str) -> None:
    ...
    if actual != expected:
        raise DenvError(f"digest mismatch for {path.name}\n"
                        f"  expected {expected}\n  actual   {actual}")
...
def run_logged(what: str, cmd: list[str], env: dict[str, str],
    ...
    result = subprocess.run(
        cmd, env=env, input=stdin_text, text=True,
...
def ensure_micromamba(ctx: Ctx) -> None:
    ...
    expected = archive.with_name(archive.name + ".sha256") \
        .read_text().split()[0]
...
def conda_create(ctx: Ctx, step: CondaEnv) -> None:
    ...
    log(f"conda {step.prefix_rel} ({step.platform.value}): "
        + " ".join(step.packages))
    run_logged(
        f"micromamba create {step.prefix_rel}",
        [str(ctx.mamba), "create", "--yes", "--quiet", "--no-rc",
         "--prefix", str(prefix), "--platform", step.platform.value,
         "--channel", "conda-forge", *step.packages],
        ctx.tool_env())
...
def _uv(ctx: Ctx) -> str:
    ...
    if found is None:
        raise DenvError("uv is required and was not found on PATH; "
...
def do_uv_venv(ctx: Ctx, step: UvVenv) -> None:
    ...
    python = layout.venv / ("Scripts/python.exe" if host.os is OS.WINDOWS
                            else "bin/python")
    ...
    run_logged("uv python install", [uv, "python", "install",
                                     *([step.version] if step.version else [])],
               env)
    if layout.venv.exists():
        shutil.rmtree(layout.venv)
    run_logged("uv venv", [uv, "venv", "--seed",
...
def do_fetch(ctx: Ctx, step: Fetch) -> None:
    ...
    (_extract_zip if step.kind == "zip" else _extract_tar)(
        archive, partial, step.strip)
    if dest.exists():
        shutil.rmtree(dest)
...
def do_android_sdk(ctx: Ctx, step: AndroidSdk) -> None:
    ...
    packages = ["platform-tools", f"platforms;android-{step.api}",
                f"build-tools;{step.api}.0.0"]
    wanted = [sdk / "platform-tools", sdk / "platforms" / f"android-{step.api}",
    ...
    subprocess.run([str(sdkmanager), "--licenses"], env=env,
                   input="y\n" * 60, text=True,
                   stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    log("installing SDK packages: " + " ".join(packages))
    run_logged("sdkmanager --install",
               [str(sdkmanager), "--install", *packages], env)
...
def do_ghcup(ctx: Ctx, step: GhcupToolchain) -> None:
    ...
    if ghc.exists() and cabal.exists():
        return
    from .catalog import conda_bin_dirs
    path = os.pathsep.join(
        [str(base / "bin"), str(layout.shims),
         *map(str, conda_bin_dirs(layout, ctx.host)),
         "/usr/bin", "/bin"])
    env = ctx.tool_env(GHCUP_INSTALL_BASE_PREFIX=str(base), PATH=path,
    ...
    if not ghc.exists():
        run_logged("ghcup install ghc",
                   [ghcup, "install", "ghc", step.ghc, "--set"], env)
    if not cabal.exists():
        run_logged("ghcup install cabal",
                   [ghcup, "install", "cabal", "recommended", "--set"], env)
...
def _aapt2_version(ctx: Ctx) -> str:
    ...
    fetch(f"{GOOGLE_MAVEN}/com/android/tools/build/aapt2/maven-metadata.xml",
          metadata)
...
def do_aapt2_shim(ctx: Ctx, step: Aapt2Shim) -> None:
    ...
    if not real.exists():
        ...
        fetch(f"{GOOGLE_MAVEN}/com/android/tools/build/aapt2/{version}/"
              f"aapt2-{version}-linux.jar", jar)
        real.parent.mkdir(parents=True, exist_ok=True)
        with zipfile.ZipFile(jar) as zf, zf.open("aapt2") as src, \
                open(real, "wb") as out:
            shutil.copyfileobj(src, out)
        real.chmod(0o755)
    write_qemu_wrapper(ctx, step.qemu_binary, step.sysroot_platform,
                       real, layout.shims / "aapt2")
    _set_gradle_property(layout, "android.aapt2FromMavenOverride",
                         str(layout.shims / "aapt2"))


def write_qemu_wrapper(ctx: Ctx, qemu_binary: str, platform: CondaPlatform,
                       real: Path, wrapper: Path) -> None:
...
def _set_gradle_property(layout: Layout, key: str, value: str) -> None:
    ...
    if properties.exists():
        lines = [l for l in properties.read_text().splitlines()
...
def do_bind_gradle(ctx: Ctx, step: BindGradleProject) -> None:
    ...
    gradle_markers = ("settings.gradle", "settings.gradle.kts",
    ...
    lines = []
    if local.exists():
        lines = [l for l in local.read_text().splitlines()
                 if not l.startswith("sdk.dir=")]
...
def provision(plan: Plan) -> list[ProbeResult]:
    layout = plan.layout
    for directory in (layout.root, layout.downloads, layout.home, layout.tmp,
                      layout.cache, layout.shims, layout.tmp,
                      layout.home / ".config"):
...
def write_activation(plan: Plan) -> None:
    layout = plan.layout
    for path, text in ((layout.activate_sh, render_sh(plan.env, plan.host)),
...
def verify(plan: Plan) -> list[ProbeResult]:
    ...
    for command in plan.probes:
        ...
        try:
            proc = subprocess.run(
                [argv0, *command[1:]], env=env, timeout=600, text=True,
                stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
        except subprocess.TimeoutExpired:
            results.append(ProbeResult(command, False, "probe timed out"))
            continue
        first = next((l for l in (proc.stdout or "").splitlines() if l.strip()),
                     "")
...
def make_shim(layout: Layout, host: Host, binary: Path,
              platform: CondaPlatform) -> Path:
    ...
    match emu:
        ...
        case QemuUser():
            pass
        case _:
            raise DenvError(f"{platform.value} binaries already run natively "
    ...
    write_qemu_wrapper(ctx, emu.qemu_binary, platform,
                       binary.resolve(), wrapper)
    ctx.save_manifest()
    return wrapper
